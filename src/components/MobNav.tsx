...
const MobNav: FC = () => {
    ...
    return (
        <AnimatePresence>
            ...
            {
                menuStatus &&
                <motion.div onClick={menuClose} initial={{ opacity: 0, }}
                    animate={{ opacity: 1 }}
                    transition={{ duration: 0.5, type: 'spring' }}
                    exit={{ opacity: 0 }}
                    className="md:hidden fixed top-0 left-0 h-screen w-full bg-black/30 backdrop-blur-lg p-4">
                    <button onClick={menuClose} className='focus:animate-spin duration-300 delay-100 hover:text-primary right-8 top-6 absolute'>
